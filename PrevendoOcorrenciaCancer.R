# Prevendo a Ocorrência de Câncer- Modelo de classificação 
## Previsão de classes, categorias


# Configurando o diretório de trabalho
# Coloque entre aspas o diretório de trabalho que você está usando no seu computador
setwd("C:/Users/deeww/OneDrive/Área de Trabalho/Ciencia de dados/Curso Formação Cientista de dados/BigData_R_Azure_ML/cap11/Classificacao")
getwd()
...
# indicar benigno.
dados <- read.csv("dataset.csv", stringsAsFactors = FALSE)##não considere nenhuma variável numérica como fator
str(dados)
View(dados)


###diagnosis é a coluna da variável target (se o cancer é benigno ou não)


## Etapa 2 - Pré-Processamento

# Excluindo a coluna ID (não é uma carcterística relevante)
...
##normalização:transformando os valores numéricos na mesma escala e distribuição normal (só quando necessário)
...
# Criando um função de normalização (PADRONIZAÇÃO)
...
# Normalizando os dados: lapply faz um loop - colocando as variáveris numéricas na mesma escala
dados_norm <- as.data.frame(lapply(dados[2:31], normalizar))
View(dados_norm)


## Etapa 3: Treinando o modelo com KNN (calcula os vizinho mais próximo à média)
...
# Criando os labels para os dados de treino e de teste (variável target - benigno e maligno)
...
modelo_knn_v1 <- knn(train = dados_treino, 
                     ...
                     k = 21)  ##21 pontos mais próximos (distância euclidiana)
...
# Usando a função scale() para padronizar o z-score - NORMALIZAÇÃO
##(porque -1: tirei a coluna alvo) - ultiza o dataset original
?scale()
...
# Etapa 7: Construindo um Modelo com Algoritmo Random Forest (CONJUNTO DE ÁRVORES DE DECISÃO)

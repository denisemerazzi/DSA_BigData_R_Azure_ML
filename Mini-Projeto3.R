...
setwd("C:/Users/deeww/OneDrive/Área de Trabalho/Ciencia de dados/Curso Formação Cientista de dados/BigData_R_Azure_ML/Miniprojeto3")
getwd()

# Instalação dos pacotes
install.packages("h2o")  ##pacote que contem o ML
install.packages("tidyverse")   ##manipulação de dados
install.packages("ggbeeswarm")   ###gráficos customizados em cima do ggplot2
...
modelo_automl <- h2o.automl(y = 'manutencao',
                            ...
                            nfolds = 4, ##validação cruzada
                            leaderboard_frame = h2o_frame_split[[2]],
                            max_runtime_secs = 60 * 2,   ##2 minutos de execução
                            include_algos = c('XGBoost', 'GBM', 'GLM'),  ##escolha dos algoritmos
                            sort_metric = "AUC")  ##métrica de ordenação (compara modelos de diferentes arquiteturas)

# Extrai o leaderboard (tabela comparativa entre os modelos - ranking)

...
setwd("C:/Users/deeww/OneDrive/Área de Trabalho/Ciencia de dados/Curso Formação Cientista de dados/BigData_R_Azure_ML/Miniprojeto 2")
getwd()
...
# Verificando ocorrência de valores NA (soma as colunas de NA)
colSums(is.na(dados)) 
...
# 4- Qual a variância da coluna idade? (implementação da fórmula matemática em SQL) R: 48.3
##NO R, utiliza-se a função var
...
# 5- Qual o gasto total com internações hospitalares por idade? (Usar groupby)
...
# 6- Qual idade gera o maior gasto total com internações hospitalares? R: 0
class(gasto_total_idade) ##é um dataframe
arrange(gasto_total_idade, desc(Gasto_Total))[1,]

# 7- Qual o gasto total com internações hospitalares por gênero?R: Feminio = 650647 e masculino = 735391
...
# 9- Para pacientes acima de 10 anos, qual a média de gasto total com internações hospitalares? (Filtro WHERE)
...
# 10- Considerando o item anterior, qual idade tem média de gastos superior a 3000? (HAVING )
...
## Coluna utiliza-se a clausula WHERE; filtrar colunas da tabela
## a clausula HAVING filtra o resultado da condição (cálculo da agregação) 
...
# Convertemos a variável para o tipo fator (categórica) e então obtemos o sumário que precisamos.
...
#  Usaremos um modelo de Regressão Linear par5a uma análise estatística.
...
##p > 0,05, ou seja, falhamos em rejeitar H0, ou seja, não existe relação entre as variáveis
## R-squared é muito pequeno (0.007898), ou seja, o modelo erra muito (porque não tem padrão nas variáveis)
